...
type
  ...
  TForm1 = class(TForm)
    ...
    TrackBar3: TTrackBar;
     ...
     procedure AddLbs(txt: tstringlist; al: tAlignment); // Создать новую надпись
...
procedure tform1.AddLbs(txt: tstringlist; al: tAlignment); // Создать новую надпись
begin
       ...
       if wr.image then   // это картинка
         begin
           ...
           repeat
             if s[i]=#1 then
                begin
                   txt.Add(copy(s, j, i-j));
                   j:=i+1;
                end;
           inc(i);
           until s[i]=#2 ;
           form1.AddLbs(txt, wr.Alignment);
