...
uses
  ...
  ExtCtrls, StdCtrls, printers, ComCtrls, Menus, Buttons, LResources,
  PrintersDlgs, RTTICtrls;
...
type
  ...
  TForm1 = class(TForm)
    ...
    TrackBar3: TTrackBar;
     ...
     procedure Say(word: string); // Записать строку в лог-файл
...
procedure tform1.Say(word: string); // Записать строку в лог-файл
var
   fl: textfile;
   fn: string;
begin
   fn := MyFolder + 'vap_prn.log';
   if word = '' then
        begin
          assignfile(fl, fn);
          rewrite(fl);
