...
type
  ...
  private
    ...
    x0,y0,x1,y1        : Integer;      // размеры бумаги
    s                  : string;       // строковая перемнная (имя формата листа)
  ...
  public
    ...
    procedure SetNewPaperSize(Name: string; xTop,yTop,xBottom,yBottom: Integer);  // добавлено
    procedure SelectPaper(Name: string); // добавлено
  end;
...
procedure tPaperSize.SetNewPaperSize(Name: string; xTop,yTop,xBottom,yBottom: Integer);
begin
  s:=Name;
  x0:=xTop;
  y0:=yTop;
...
procedure tPaperSize.SelectPaper(Name: string);
begin
     SetPaperName(Name);
end;
...
begin
  ...
  begin
    ...
    add(3,  s,          PRRect(x0, y0, x1,  y1 ), PRRect(x0,  y0,  x1,  y1));
  end else begin
    add(Length(fInternalPapers), s, PRRect(x0, y0, x1,  y1 ), PRRect(x0,  y0,  x1,  y1));
  end;
end;
